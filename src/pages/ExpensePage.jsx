...
import { CreditCard, Edit3, Plus, RefreshCw, Trash2 } from "lucide-react";
import { useState } from "react";
const money = (n) =>
  `₹ ${Number(n || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
const today = () => new Date().toISOString().slice(0, 10);
export default function ExpensePage({ type, title }) {
  const hasPaymentStatus = ["smc", "other"].includes(
  ...
  const [create, { isLoading: creating }] = useCreateExpenseMutation(),
    [update, { isLoading: updating }] = useUpdateExpenseMutation(),
    [remove] = useDeleteExpenseMutation(),
    [addPayment, { isLoading: adding }] = useAddExpensePaymentMutation(),
    [deletePayment] = useDeleteExpensePaymentMutation();
  const rows = data?.expenses || [];
  const openNew = () => {
    setForm({ date: today(), amount: "", remarks: "", expenseType: type });
  ...
  const submit = async (e) => {
    ...
    try {
      if (modal.expense)
        ...
        await create({
          expenseData: { ...form, amount: Number(form.amount) },
        }).unwrap();
      setToast(modal.expense ? "Expense updated" : "Expense added");
      setModal(null);
    } catch (err) {
  ...
  const del = async (id) => {
    ...
    try {
      ...
      setToast(e?.data?.message || "Unable to delete");
    }
  };
  const openPay = (e) =>
    setModal({ payment: e, amount: "", date: today(), remarks: "" });
  const savePay = async (e) => {
    e.preventDefault();
    try {
      await addPayment({
        id: modal.payment._id,
        amount: Number(modal.amount),
        paymentDate: modal.date,
  ...
  const delPay = async (exp, pid) => {
    if (!confirm("Remove this payment?")) return;
    try {
      await deletePayment({ id: exp._id, paymentId: pid }).unwrap();
      setToast("Payment removed");
    } catch (e) {
      setToast(e?.data?.message || "Unable to remove payment");
    }
  };
  return (
    <>
      ...
      <div className="panel table-panel">
        <div className="toolbar">
          <div>
            <b style={{ fontSize: 12 }}>{data?.pagination?.totalData || 0}</b>
            <span style={{ fontSize: 10, color: "#94a3b8" }}> records</span>
          </div>
          <button
            className="small-btn"
              ...
              <table className="table">
                ...
                <tbody>
                  {rows.length ? (
                    ...
                    <tr>
                      <td colSpan={hasPaymentStatus ? 8 : 5}>
      ...
      {modal?.expense !== undefined && (
        <AppModal
          title={`${modal.expense ? "Edit" : "Add"} expense`}
                ...
                {modal.payment.payments.map((p) => (
                  <div key={p._id}>
                    ...
                    <button
                      type="button"
                      className="small-btn"
                      onClick={() => delPay(modal.payment, p._id)}
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
